...
export function CollaboratorList({
    ...
    React.useEffect(() => {
        const fetchCollaborators = async () => {
            try {
                ...
                const coldata: User[] =
                    type === 'managers' ? data.managers : data.translators;
    ...
    const handleRemoveCollaborator = async (id: number) => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/remove-project-group-user?group=${type}&project_name=${projectName}&user_id=${id}`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
        ...
        <div className="space-y-4">
            <div className="flex items-center justify-between">
                <h4 className="text-sm font-semibold">
                    {type === 'managers' ? 'Project Managers' : 'Translators'}
                </h4>
            ...
            {filteredCollaborators.length === 0 ? (
                <div className="p-4 text-center text-sm text-muted-foreground">
                    No available{' '}
                    {type === 'managers' ? 'Managers' : 'Translators'}
                </div>
            ) : (
                <div className="rounded-md border">
                    <div className="border-b p-4">
                        <div className="flex justify-between items-center">
                            {(isadmin || type === 'translators') && (
                                <Checkbox
                                    checked={
                                        selectedIds.size ===
