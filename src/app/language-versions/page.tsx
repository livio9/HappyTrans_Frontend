...
function LanguageVersionsContent() {
    ...
    const handleExportProject = async (languageCode: string) => {
        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_API_BASE_URL}/export-project?project_name=${projectName}&language_id=${languageCode}`,
                {
                    method: 'GET',
                    headers: {
    ...
    const handleCommunityNavigation = () => {
        router.push(
            `/community-forum?project=${encodeURIComponent(projectName)}`
        );
