        ...
        if (1) {
            try {
                const LLMResponse = await fetch(
                    `${process.env.NEXT_PUBLIC_API_BASE_URL}/ai-suggestions?target_language=${targetLanguage}&idx_in_project=${idx_in_project}&source_language=${sourceLanguage}&project_name=${projectName}`,
                    {
                        method: 'GET',
                        headers: {
                ...
                if (LLMResponse.ok) {
                    ...
                    if (LLMData.error) {
                        ...
                        LLMData.map(
                            (llm: { name: string; suggestion: string }) => {
                                suggestions.push({
                                    source: llm.name,
