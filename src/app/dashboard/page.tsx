...
const Dashboard: React.FC = () => {
    ...
    const [activeProjectsCount, setActiveProjectsCount] =
        React.useState<number>(0);
    const [languageVersions, setLanguageVersions] = React.useState<
        LanguageVersion[]
    >([]);
    ...
    const [projectActivities, setProjectActivities] = React.useState<{
        [projectName: string]: ActivityLog[];
    }>({});
     ...
     * 获取项目的语言版本信息
     */
    const fetchLanguageVersions = React.useCallback(
        async (projectName: string): Promise<LanguageVersion[] | null> => {
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    const languageVersions: LanguageVersion[] = languages.map(
                        (lang: {
                            ...
                            const progress =
                                typeof lang.selected_entries_ratio === 'number'
     ...
     */
    const fetchActivityLogs = React.useCallback(
        async (projectName: string): Promise<ActivityLog[]> => {
            try {
                ...
                if (response.ok) {
                    ...
                    console.error(
                        `Failed to fetch activities for project ${projectName}`
                    );
                    setError(
                        `Failed to fetch activities for project ${projectName}.`
                    );
    ...
    const initializeDashboard = React.useCallback(async () => {
        ...
        if (profileData) {
            const managed = Array.isArray(profileData.managed_projects)
                ? profileData.managed_projects
                : [];
            const translated = Array.isArray(profileData.translated_projects)
                ? profileData.translated_projects
                : [];
            const projects = [...managed, ...translated];

            // 计算唯一项目数
            const uniqueProjectIds = new Set(
                projects.map((p: any) => p.id).filter((id: any) => id)
            );
            ...
            const languageVersionsArray = await Promise.all(
                projects.map(async (proj: any) => {
                    const projectName =
                        proj && typeof proj.name === 'string' ? proj.name : '';
                    if (!projectName) {
                        return null;
                    }
            ...
            if (allLanguageVersions.length > 0) {
                const totalProgress = allLanguageVersions.reduce(
                    (sum: number, lang: LanguageVersion) => sum + lang.progress,
                    0
                );
                const averageProgress =
                    totalProgress / allLanguageVersions.length; // 假设 selected_entries_ratio 已是 0-100
            ...
            // 设置每个项目的最近三个活动
            const projectActivitiesMap = activitiesResults.reduce(
                (acc, { projectName, activities }) => {
                    if (projectName) {
    ...
    const displayProgress = Number.isFinite(translationProgress)
        ...
        <div>
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
                ...
                <Card>
                    ...
                    <CardContent>
                        <Progress
                            value={
                                Number.isFinite(translationProgress)
                                    ? translationProgress
                                    : 0
                            }
                            className="w-full"
                ...
                <Card>
                    <CardHeader>
                        ...
                        <CardDescription>
                            Total number of your accepted entries
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        <div className="flex items-center space-x-2">
                            ...
                            <Medal
                                level={
                                    getAcceptedEntriesLevel(acceptedEntries)
                                        .level
                                }
                                color={
                                    getAcceptedEntriesLevel(acceptedEntries)
                        ...
                        <p className="text-sm text-gray-600">
                            Your accepted contributions and medal
                        </p>
                    </CardContent>
                </Card>
            </div>
            ...
            <Card className="mt-6">
                <CardHeader>
                    ...
                    <CardDescription>
                        Recent updates for each of your projects
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {Object.keys(projectActivities).length === 0 ? (
                        <p className="text-sm text-gray-500">
                            No project activities available.
                        </p>
                    ) : (
                        Object.entries(projectActivities).map(
