...
const Comment = ({ comment, fetchComments }: CommentProps) => {
    ...
    const [userCache, setUserCache] = useState<Map<string, UserType>>(new Map()); // 缓存用户信息，避免重复请求
    ...
    const fetchUserInfo = useCallback(async (username: string): Promise<UserType | undefined> => {
        if (userCache.has(username)) return userCache.get(username);

        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/profile?username=${username}`, {
                method: "GET",
                headers: {
    ...
    const fetchCommentById = useCallback(async (commentId: number): Promise<CommentType | undefined> => {
        ...
        try {
            ...
            setCommentCache((prev) => new Map(prev).set(commentId, fetchedComment));
    ...
    useEffect(() => {
        const fetchAndSetUser = async () => {
            if (comment.created_by && !comment.user) { // 确保只有在 created_by 存在且没有获取到用户信息时才请求
    ...
    const [replyCount, setReplyCount] = useState<number>(countTotalReplies(comment.replies || []));
    ...
    // 得到扁平化回复列表
    const flattenReplies = useCallback((replies: CommentType[]): FlattenedReply[] => {
        ...
        const traverse = (reply: CommentType, parentUsername?: string) => {
            ...
            if (reply.replies && reply.replies.length > 0) {
                reply.replies.forEach(childReply => traverse(childReply, reply.user?.username));
            }
        };
        ...
        try {
            ...
            if (parentComment && parentComment.created_by) {
                ...
                if (!userCache.has(parentComment.created_by)) {
                    const parentUser = await fetchUserInfo(parentComment.created_by);
                    return parentUser?.username || "Anonymous";
                } else {
                    return userCache.get(parentComment.created_by)?.username || "Anonymous";
                }
            }
        } catch (error) {
            console.error("Failed to get parent comment user information:", error);
        }
    ...
    const submitEdit = async () => {
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/comments/${comment.id}/`, {
                method: "PATCH",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Token ${token}`,
            ...
            setReplies(prevReplies => prevReplies.map(reply =>
                reply.id === comment.id ? { ...reply, content: updatedComment.content, updated_at: updatedComment.updated_at } : reply
            ));
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            // 从状态中移除该评论
            setReplies(prevReplies => prevReplies.filter(reply => reply.id !== comment.id));
    ...
    const submitReply = async () => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/comments/?discussion_id=${discussion_id}`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Token ${token}`,
    ...
    const handleDeleteReply = async (replyId: number) => {
        ...
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}/comments/${replyId}/`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Token ${token}`,
                    "X-CSRFToken": csrfToken || "",
    ...
    const ReplyItem = memo(({ index, style }: ListChildComponentProps) => {
        ...
        return (
            <div className="flex items-start space-x-4 ml-8 mb-6">
                {/* 用户头像 */}
                <UserAvatar username={replyUser?.username || "Anonymous"} size="sm" />
                {/* 右侧内容 */}
                <div className="flex-1 w-full"> 
                    {/* 用户名 */}
                    <div className="flex items-center space-x-2">
                        <CardTitle className="text-base text-gray-600">{replyUser?.username || "Anonymous"}</CardTitle>
                        {parentUsername && (
                            <span className="text-xs text-gray-500 ml-2">
                                reply to <span className="font-semibold">{parentUsername}</span>
                            </span>
                        )}
                    </div>
                    {/* 评论内容 */}
                        ...
                        {isReplyingToReply && (
                            <div className="absolute left-0 mt-2 pl-4 w-full bg-white border-none rounded shadow-lg z-10">
                                ...
                                <div className="mt-2 flex space-x-2">
                                    <Button onClick={handleReplyToReply} variant="ghost">
                                        Send
                                    </Button>
                                    <Button onClick={() => setIsReplyingToReply(false)} variant="ghost">
                                        Cancel
                                    </Button>
                                </div>
                            </div>
                    ...
                    <div className="mt-2 text-xs text-gray-500">
                        {formatDistanceToNow(new Date(reply.updated_at), { addSuffix: true })}
                    </div>
                </div>
    ...
    return (
        <Card className="mb-4 rounded-lg shadow-lg">
            <CardHeader className="flex items-start space-x-4 w-full"> {/* 确保宽度填满 */}
                <div className="flex items-start space-x-6 w-full"> {/* 确保此 div 使用完整的宽度 */}
                    {/* 头像 */}
                    <UserAvatar username={commentUser?.username || "Anonymous"} size="md" />

                    {/* 右侧内容 */}
                    <div className="flex-1 w-full"> {/* 确保右侧内容占满剩余空间 */}
                        {/* 用户名 */}
                        <div className="flex items-center space-x-2">
                            <CardTitle className="text-lg text-gray-600 font-semibold">{commentUser?.username || "Anonymous"}</CardTitle>
                        </div>
                        ...
                        <div className="mt-1 text-base">
                            {isEditing ? (
                                <div>
                                    ...
                                    />
                                    <div className="mt-2 flex space-x-2">
                                        <Button onClick={submitEdit} variant="ghost" className="text-blue-500">Save</Button>
                                        <Button onClick={cancelEdit} variant="ghost" className="text-gray-500">Cancel</Button>
                                    </div>
                                </div>
                            ) : (
                        ...
                        <div className="mt-2 text-xs text-gray-500">
                            ...
                            {comment.updated_at && comment.updated_at !== comment.created_at && (
                                <> (updated at {formatDistanceToNow(new Date(comment.updated_at), { addSuffix: true })})</>
                            )}
                        </div>
                    </div>
                </div>
            ...
            <CardFooter className="flex items-center justify-between">
                <div className="flex items-center space-x-4 ml-20">
                    ...
                    <Button variant="ghost" size="sm" onClick={toggleReplyExpansion}>
                        {showReplies ? "Hide Replies" : `View More (${replyCount})`}
                    </Button>
                </div>
            </CardFooter>
            ...
            {isReplying && (
                <div className="mt-2 pl-8">
                    ...
                    />
                    <div className="mt-2 flex space-x-2">
                        <Button onClick={submitReply} variant="ghost" className="text-blue-500">Send</Button>
                        <Button onClick={cancelReplying} variant="ghost" className="text-gray-500">Cancel</Button>
                    </div>
                </div>
            )}
