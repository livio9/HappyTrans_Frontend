...
function TranslationInterfaceContent() {
    ...
    const canSelect =
        user?.role === 'admin' ||
        (user && projectName && projectManaged?.includes(projectName));
    ...
    const handleSaveAndContinue = async () => {
        ...
        router.push(
            `/translation-interface?project_name=${projectName}&language_code=${languageCode}&idx_in_language=${currentIndex + 1}`
        ); // 使用router跳转
    ...
    const handleRemoveTag = async (tag: string) => {
        if (
            !window.confirm(`Are you sure you want to delete the tag "${tag}"?`)
        ) {
            return;
        }
    ...
    const handleProjectEntries = () => {
        router.push(
            `/Entries?project_name=${encodeURIComponent(projectName || '')}&language_code=${encodeURIComponent(languageCode || '')}`
        );
    };
